...
- [x] **Entitlement is one flag, two tiers ✅ 2026-08-01.** Students get the identical feature set, so there is no separate student gate — every paid feature asks `isPro`, and `tier` (`pro`/`edu`) only drives the badge and support copy. A parallel student entitlement would mean each future paid feature has to remember two flags, and eventually one would not. This matches the Phase 2 edu design: verifying a student issues a normal licence key at 100% off, so a verified student is a licence holder like any other and no student data reaches the client. Demo keys: DEMO-1234 (Pro), EDU-1234 (student).
